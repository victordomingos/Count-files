...
- [Аргументи CLI](#Аргументи-cli)
   ...
   - [Пошук файлів за шаблоном](#Пошук-файлів-за-шаблоном)
...
- Пошук файлів за шаблоном (використовуючи `-fm` або `--filename-match`):  
Результатом пошуку буде список шляхів до файлів, які точно відповідають вказаному шаблону з врахуванням регістру.
...
Для пошуку файлів за розширенням (використовуючи аргументи `--filename-match` або `--file-extension`) 
...
`[seq]` - позначає збіг з будь-яким із символів у послідовності, `[!seq]` - позначає збіг з будь-якими символами крім вказаних у послідовності.

Також доступне отримання додаткової інформації про розмір кожного знайденого файлу та короткий попередній перегляд для текстових файлів (аргументи `--file-sizes`, `--preview`, `--preview-size`).  
За необхідності, можна вказати один чи декілька загальних аргументів: 
`path`, `--all`, `case-sensitive`, `--no-recursion`.  
Приклад: `count-files --filename-match *.????`

Див. також [Пошук файлів за шаблоном: список шляхів до файлів](https://github.com/victordomingos/Count-files/tree/master/docs/documentation_ua/examples.md#Пошук-файлів-за-шаблоном-список-шляхів-до-файлів)
...
(використовуючи аргументи `--filename-match` або `--file-extension`)
