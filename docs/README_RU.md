...
Для работы приложения необходима версия Python 3.6+. На данный момент необходимая зависимость:
...
По умолчанию файлы будут подсчитаны рекурсивно. Результатом будет таблица, показывающую частоту для каждого расширения файла (например: .txt, .py, .html, .css) и общее количество найденных файлов.
...
По умолчанию используется рекурсивный способ подсчета или поиска файлов. Укажите необязательный аргумент `-nr` или` -no-recursion` если нет необходимости в информации о файлах из субдиректорий.
...
При поиске по определенному расширению доступен предварительный просмотр текстовых файлов.  
...
По умолчанию результатом поиска по определенному расширению является список с
 полными путями найденных файлов. Если вам нужно только общее количество файлов, используйте аргумент `-nl` или `--no-list`.
...
Обратная связь доступна по умолчанию для подсчета файлов(с таблицей и без нее) и для поиска файлов по расширению (режим просмотра без списка). Необязательный аргумент `-nf` или` --no-feedback` отключает индикатор работы программы. Использование `--no-feedback` и` --no-list` позволяет немного ускорить обработку файлов.

По умолчанию таблица будет отсортирована по частоте. Если нужна сортировка по алфавиту добавьте аргумент `-alpha` или `--sort-alpha`.
...
Подсчет всех файлов в текущем рабочем каталоге, исключая скрытые файлы и папки и без рекурсии:
